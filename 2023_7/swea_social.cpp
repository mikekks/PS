        ...
        if (rand_val < get_feed_ratio && total_post_cnt > 0)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                if (ans_pIdList[i] != pIdList[i])
                {
                    printf("this!!!!\n");
                    for(int i=0; i<10; i++){
                        printf("%d %d | ", ans_pIdList[i], pIdList[i]);
                    }
                    ret = false;
                }
            }
...
{
    follows[uID1][uID2] = timestamp;
    
}

void makePost(int uID, int pID, int timestamp)
{
    Post tmp = {0, timestamp, pID, uID};
    posts.push_back(tmp);
...
void getFeed(int uID, int timestamp, int pIDList[])
{
    ...
    priority_queue<Post, vector<Post>, cmp> min_q;
    //int cnt = 0;
    
    int k = 0;
    for(k=(int)posts.size()-1; k>=0; k--){
        if(timestamp - posts[k].time > 1000) break;
        if(follows[uID][posts[k].uId] == 0) continue;
    ...
    if(min_q.size() >= 10){
        ...
        while(i < 10){
            if(k < 0) break;
            if(follows[uID][posts[k].uId] == 0){
                k--;
                continue;
            }
            
            pIDList[i++] = posts[k--].pId;
        }
    }
